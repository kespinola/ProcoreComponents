...
var ProgressCircle = React.createClass({
  render: function() {
    ...
    var percentage =  parseInt(this.props.completion.percentage);
      ...
      <div className={classes} data-progress={percentage}>
        ...
        <div className="inset">
          <div className="percentage-container">
            <div className="numbers"><span className="number">-</span>{numbers}</div>
          </div>
        </div>
      </div>
    );
  ...
  renderPercentageNumbers: function() {
    return _(101).times(function(pct){
      return (<span className="number">{pct}<sub className="circle-percentage">%</sub></span>);
    });
  }
});
