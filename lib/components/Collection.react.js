...
var Collection = React.createClass({
  ...
  shouldComponentUpdate(newProps, newState){
    return this.state.items.length != newState.items.length || this.state.items[0].position != newState.items[0].position
  },
  ...
  componentWillReceiveProps(newProps){
    console.log('receiving new props to Collection', newProps);
