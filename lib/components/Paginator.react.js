        ...
        <span 
          ...
          className="previous_page">
            ...
            { _.range(minRange, maxRange).map(num => {
              if (page === num) { return(<em className="current">{num+1}</em>); }
              return(<a onClick={onPageChange.bind(null, num)}>{num+1}</a>);
            })}
        <span 
          onClick={onPageChange.bind(null, Math.min(maxRange-1, page+1))} 
          className="next_page">
            next
          </span>
      </div>
